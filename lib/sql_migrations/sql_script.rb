module SqlMigrations
  ...
  class SqlScript
    ...
    def self.find(db_names, type)
      scripts = []
      Find.find(Dir.pwd) do |path|
        db_names = db_names.is_a?(Array) ? db_names : [db_names]
        file_date, file_time, file_name, file_db = file_options(db_names, type, path)
        next unless file_name
        scripts << new(path, date: file_date, time: file_time,
                             name: file_name, db_name: file_db)
      end
    ...
    def self.file_options(db_names, type, path)
      ...
      [file_opts[1], file_opts[2], file_opts[3], file_database]
    end
